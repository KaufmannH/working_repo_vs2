...
# importend from config file (move back later)
SelectedTissues <- "Liver"
impose_clusters <- FALSE
testrun <- TRUE
n_cells <- 10000 # only used if testrun == TRUE
...
# Set Input folder
#input_path_to_so <- file.path('/home/hkaufm49/analyses/Spleen_10X/data/prepped_for_pipeline')
...
out_folder <- "data/processed/fast_100000_cells"    
...
#for (tissue in SelectedTissues) { # generalize tissue to input file (with different possible input file formats?)
  ...
  tissue <- SelectedTissues # change if more tissues are run
  gc()

  files <- list.files(input_path_to_so)

  for (f in files) { # implement the loop later
    #f <- files[1]
