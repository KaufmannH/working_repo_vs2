...
res_var_bootstrap <- function(
  ...
  # CRITICAL: disable sctransform's internal parallelism to prevent
  # nested forking (mclapply inside dopar workers), which causes SIGPIPE
  ...
  # helper to restrict cell_attr to sampled cells
  ...
  for (cl in cl_levels) {
    ...
    # parallel over iterations (reproducible via doRNG)
    iter_tbl <-
      foreach::foreach(iter = seq_len(n_cycles),
        ...
        # slice counts; set up matching vst.out
        umi_subset     <- umi_cl[, sel, drop = FALSE]
        vst_out_subset <- .subset_vst(vst_base, sel_cells)

        # assign unique names so get_residual_var doesn't choke on duplicates
