...
Sys.setenv(RETICULATE_PYTHON = "/home/hkaufm49/anaconda3/envs/regnoise_seurat5_env/bin/python")
library(reticulate)
library(tidyverse)
library(Seurat)
library(patchwork)
...
source('src/FunctionLib_seurat5_improved.R')
...
out_folder <- "data/processed/added_seurat5_improved"          
...
#for (tissue in SelectedTissues) { # generalize tissue to input file (with different possible input file formats?)
  ...
  for (age_group in AgeSexGroups) {
    ...
    #source('src/FunctionLib_seurat5_integrated.R')
