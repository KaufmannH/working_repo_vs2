...
ggsave(filename = paste0("droplet_analysis/plots/all_ages/dotplot_tissue.jpeg"), width = 8, height = 6, dpi = 300)
...
hist <- ggplot(enrich_score, aes(x = enrichment_score)) +
  ...
  theme_classic()
ggsave("droplet_analysis/plots/all_ages/hist_enrichment_score.png", plot = hist, width = 8, height = 6)
...
ggsave("droplet_analysis/plots/all_ages/barchart_cell_type_combo.png", plot = bar_cell_type_combo_plot)
...
ggsave("droplet_analysis/plots/all_ages/barchart_tissue_combo.png", plot = bar_tissue_combo_plot)



# AGE
# barplot age
age_plot <- ggplot(perc_age, aes(x = age, y = percent, fill = as.factor(cluster_enriched))) +
  geom_bar(stat = "identity", position = "fill") + 
  scale_y_continuous(labels = scales::percent_format()) + 
  scale_fill_manual(values = palette[c(2, 1)]) +
...
# barplot age num clusters
age_numb_clusters_plot <- ggplot(perc_age, aes(x = age, y = num_clusters)) +
  geom_bar(stat = "identity") +  #  position = "fill" makes it a stacked bar plot
  scale_y_continuous(limits = c(0, 150)) + 
  scale_fill_manual(values = palette[c(2)]) +
  labs(
...
bar_age_combo_plot <- (age_numb_clusters_plot / age_plot) + plot_layout(heights = c(1, 1))
ggsave("droplet_analysis/plots/all_ages/barchart_age_combo.png", plot = bar_age_combo_plot)
