...
# load tagged df
df_main_with_markers <- readRDS("facs_analysis/data/df_with_markers.rds")
long_df_main_with_markers <- readRDS("facs_analysis/data/long_df_with_markers.rds")
df_long_strat <- readRDS("facs_analysis/data/stratified_df.rds")

check <- df_main_with_markers %>%
...
df_plot_1 <- df_main_with_markers %>%
  filter(!is.na(cell_type), !is.na(cluster_id)) %>%
  group_by(cluster_id, cell_type, gene_variability, lps_stim) %>%
  summarise(count = n(), .groups = 'drop')
head(df_plot_1)

my_colors <- c("TRUE" = "#556967","FALSE" = "#5AB4AC" )
df_plot_1$gene_variability <- factor(df_plot_1$gene_variability, levels = c("HVG", "Other", "LVG","Response gene not expressed"))

barplot_relative <- ggplot(df_plot_1, aes(x = gene_variability, y = count, fill = lps_stim)) +
...
category_counts <- cleaned_long_df_main_with_markers %>%
  filter(values == TRUE) %>%
  filter(!is.na(cell_type), !is.na(cluster_id)) %>%
  group_by(cluster_id,cell_type, gene_variability, category) %>%
  summarise(count = n(), .groups = "drop") 
category_counts
...
category_counts$gene_variability <- factor(category_counts$gene_variability, levels = c("HVG", "Other", "LVG", "Response gene not expressed"))

barplot_relative <- ggplot(category_counts, aes(x = gene_variability, y = count, fill = category)) +
  geom_bar(stat = "identity", position = "fill" ) +
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_parsed) +
  labs(x = "Gene Set",
       y = "Percentage of Genes (%)",
...
barplot_relative <- ggplot(category_counts, aes(x = category, y = count, fill = gene_variability)) +
  geom_bar(stat = "identity", position = "fill" ) +
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_parsed) +
  labs(x = "Gene Set",
       y = "Percentage of Genes (%)",
...
ear <- unique(subset(df_main_with_markers, lps_stim_early == TRUE)$gene)
late <- unique(subset(df_main_with_markers, lps_stim_late == TRUE)$gene)
...
barplot_relative <- ggplot(df_plot_1, aes(x = lps_stim, y = count, fill = gene_variability)) +
  geom_bar(stat = "identity", position = "fill", width = 0.6) +
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_parsed) +
  labs(x = "Gene type",
       y = "Fraction of genes",
...
cleaned_long_df_main_with_markers <- long_df_main_with_markers %>%
   filter(values == TRUE) 


#my_colors <- c("Marker Gene" = "#B8C7C7", "Other" = "#556967", "Immune Response Gene" = "#E0D5C8", "Stable Housekeeping Gene" = "#AE9982", "Housekeeping Gene" = "#5AB4AC", "Response gene not expressed" = "#A4C089")
#summary_table$gene_variability <- factor(summary_table$gene_variability, levels = c("HVG", "Other", "LVG"))


 boxplot_expression <- ggplot(cleaned_long_df_main_with_markers, aes(x = category, y = gmean, fill = category)) +
  geom_boxplot(alpha = 0.6, outlier.colour = "black", outlier.alpha = 0.1, size = 0.1) + 
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_parsed) +
  labs(y = "Expression Level") +
  theme_classic() +
...
long_df_main_with_markers <- long_df_main_with_markers_prep %>%
  pivot_longer(
...
cleaned_long_df_main_with_markers$category <- factor(cleaned_long_df_main_with_markers$category, 
...
summary_table <- cleaned_long_df_main_with_markers %>%
  group_by(cell_type, cluster_id, gene_variability, category) %>%
  summarise(count = n(), .groups = 'drop') 
...
barplot_relative <- ggplot(summary_table, aes(x = gene_variability, y = count, fill =  category)) +
...
barplot_relative <- ggplot(summary_table, aes(x = category, y = count, fill =  gene_variability)) +
  geom_bar(stat = "identity", position = "fill") +  # Using 'fill' for 100% stacking
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_parsed) +
  labs(x = "Gene Set",
       y = "Percentage of Genes (%)",
...
long_df_main_with_markers <- df_main_with_markers %>%
  pivot_longer(
...
stratified_df <- df_main_with_markers %>%
  mutate(low_low = gene_variability == 'LVG' & expression_level == 'Lowly Expressed') %>%
  mutate(high_high = gene_variability == 'HVG' & expression_level == 'Highly Expressed') %>%
  mutate(high_low = gene_variability == 'HVG' & expression_level == 'Lowly Expressed') %>%
  mutate(low_high = gene_variability == 'LVG' & expression_level == 'Highly Expressed')
stratified_df

long_df_main_with_markers1 <- stratified_df %>%
  pivot_longer(
...
cleaned_long_df_main_with_markers1 <- long_df_main_with_markers1 %>%
   filter(values == TRUE) 


long_df_main_with_markers2 <- cleaned_long_df_main_with_markers1 %>%
  pivot_longer(
    cols = c(
...
cleaned_long_df_main_with_markers2 <- long_df_main_with_markers2 %>%
   filter(values2 == TRUE) 
cleaned_long_df_main_with_markers2



summary_table <- cleaned_long_df_main_with_markers2 %>%
...
report <- cleaned_long_df_main_with_markers2 %>%
...
long_df_specific <- df_main_with_markers %>%
  ...
  pivot_longer(
    cols = c('Marker Gene' , "Housekeeping Gene Lin", "Housekeeping Gene", "LPS response early", "LPS response late", "IFNy response", "Il10 response" , "Il4 response", "TNF response", "Other"),
    names_to = "category",
    values_to = "values"
  ) 
...
output_dir <- "facs_analysis/plots/3_m/Test_6"
...
colnames(long_df_main_with_markers)


# just macrophages
unique(df_long_strat$cell_type)

df_agg_macs <- df_long_strat %>% 
  filter(cell_type == "Macrophage") %>%
  group_by(category, cluster_id, gene) #%>%
  #summarise(gmean = mean(gmean, na.rm = TRUE), .groups = "drop")
...
output_dir <- "facs_analysis/plots/3_m/Test_6/Macs"
...
# TEST 7
head(df_main_with_markers)

# violin plots and sctrans figure reproduction

bins <- df_main_with_markers %>%
...
# densities
print(long_df_main_with_markers[, 6:10])

unique(long_df_main_with_markers$category)

density_df <- long_df_main_with_markersf |>
  filter(cluster_id == c( "Marrow (male, cluster 14)" , "Marrow (female, cluster 10)" )) |>
  #filter(!category %in% c("LPS response early", "LPS response late", "Immune Response Gene" ,  "Housekeeping Gene" , "Cytokine response" , "TLR response", "TNF response" ))
  filter(!category %in% c("LPS response",  "Housekeeping Gene"  ))
unique(density_df$category)


density_plot <- ggplot(density_df, aes(x = gmean, y = category, colour = category, fill = category)) +
 # geom_density(position = "stack", alpha=0.3) +
  ggridges::geom_density_ridges(scale = 1.5, alpha = 0.8, colour = "white") +
  facet_wrap(~paste(cell_type, cluster_id, sep = " - "), scales = "free", labeller = label_value) +
  labs(x = "Mean expresson in cluster",
       y = "Density",
       fill = "Gene Set",
       colour = "Gene Set") +
  theme_classic() +
...
ggsave('facs_analysis/plots/3_m/Test_10/density_plot1.png', plot = density_plot, width = 12, height = 7, dpi = 300)
