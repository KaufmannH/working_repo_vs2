...
lung_male <- readRDS("facs/cell_level/processed_10_22/3m/Lung_3m_male_processed.rds") 
lung_female <- readRDS("facs/cell_level/processed_10_22/3m/Lung_3m_female_processed.rds")
lung_female
main_seurat <- merge(lung_female, y = lung_male, add.cell.ids = c("lung_female", "lung_male"))



path <- "facs/cell_level/processed_10_22/3m"
...
df_facs <- read.csv("facs_analysis/data/combined_data.csv") 


df_facs_filtered <- df_facs |>
  mutate(age = str_extract(age, "\\d+m")) |>  
  mutate(age = as.numeric(str_remove(age, "m"))) |> 
  filter(age == 3) |>
  rename(cell_type = manual_final) |> 
  filter(!(is.na(cell_type)))
colnames(df_facs_filtered)
unique(df_facs_filtered$cell_type)


# map cell type
cluster_map <- df_facs_filtered[, c("cluster_id", "cell_type")]
...
# save
#saveRDS(main_seurat, "facs_analysis/data/fused_seurat_facs_3m.rds")
...
# load cell types
cell_type_names <-  read_xlsx('facs_analysis/data/cell_type_list.xlsx')
...
#saveRDS(immune_seurat, "facs_analysis/data/immune_seurat_facs_3m.rds")
