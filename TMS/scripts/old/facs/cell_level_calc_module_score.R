...
# add lps
lps_data <- read_excel("facs_analysis/data/Bhatt_2012_data.xlsx")
...
sperm_table <- read_excel('facs_analysis/data/GO_gene_lists/GO_sperm_dna_condensation.xlsx')
sperm_list <- sperm_table |> pull(Symbol) |> unique() |> sort()
gene_set_list[["mm"]][["Sperm"]] <- sperm_list


# add the meiosis signature for basal level
meiosis_table <- read_excel('facs_analysis/data/GO_gene_lists/GO_G1_M_transition_meiosis.xlsx')
...
oocyte_table <- read_excel('facs_analysis/data/GO_gene_lists/GO_oocyte_maturation.xlsx')
oocyte_list <- oocyte_table |> pull(Symbol) |> unique() |> sort()
gene_set_list[["mm"]][["Oocyte"]] <- oocyte_list


# load the mouse/human conversions
m2h <- read.delim("facs_analysis/data/20200307_ensembl/mouse.txt",  header = FALSE, sep = "\t")
colnames(m2h) <- c("mouse", "human")
h2m <- read.delim("facs_analysis/data/20200307_ensembl/human.txt", header = FALSE, sep = "\t")
colnames(h2m) <- c("human", "mouse")
head(h2m)


# add autimmune pleiotropic loci
autimmune_df <- read_excel('facs_analysis/data/Marquez_pleiotropic_genes.xlsx')
...
# load seurat object
# all cell types
main_seurat <- readRDS("facs_analysis/data/fused_seurat_facs_3m.rds")
# immune cell types
main_seurat <- readRDS("facs_analysis/data/immune_seurat_facs_3m.rds")
#
...
pdf("facs_analysis/cell_level/plots/module_score.pdf", width = 5, height = 6)
...
output_dir <- "facs_analysis/cell_level/plots/negativ_control"
...
output_dir <- "facs_analysis/cell_level/plots/negative_contol"
for (cat in unique(df_agg_macs$category)) {
  df_subset <- df_agg_macs %>% filter(category == cat)
