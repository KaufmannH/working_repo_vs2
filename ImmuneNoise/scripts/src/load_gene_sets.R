...
load_filter_og_df <- function(data_source, selected_tissue) {
  ...
  if (data_source == "facs") {
    ...
    immune_cell_type_list <- read_xlsx("ImmuneNoise/facs/data/cell_type_list_selected.xlsx") |> 
    ...
    immune_cell_type_list <- read_xlsx("ImmuneNoise/droplet/data/cell_type_list_selected.xlsx") |> 
      filter(!is.na(innate_cell)) |> 
      pull(innate_cell)
...
tag_hk_lin_genes <- function(df) {
  ...
  housekeeping_lin_list <- read_lines("ImmuneNoise/reference_gene_sets/scHK_human.symbols.txt")

  # load the mouse/human conversions
  m2h <- read.delim("ImmuneNoise/reference_gene_sets/20200307_ensembl/mouse.txt", header = FALSE, sep = "\t")
...
tag_innate_response_genes <- function(df) {
  ...
  mouse_innate_genes_df_raw <- read.csv('ImmuneNoise/reference_gene_sets/mouse_innate_genes.csv')
...
tag_marker_genes <- function(df) {
  markers_raw  <- read.delim("ImmuneNoise/reference_gene_sets/lineage_specific_markers.csv", header = TRUE, sep = "\t")
...
tag_mac_immune_response <- function(df) {

 #immune_df <- read.table("ImmuneNoise/reference_gene_sets/mac_immune_process.txt", header = FALSE, sep = "\t")
 #immune_gene_list <- immune_df[[3]] |> str_to_lower() |> str_to_title()
  

  lps_df_raw <- read_excel("ImmuneNoise/reference_gene_sets/Bhatt_2012_data.xlsx")
  colnames(lps_df_raw)[colnames(lps_df_raw) %in% c("Probe...1", "Probe...2", "Promoter Class", "...4")] <- c("gene", "gene_id", "promoter_class", "group")
  lps_list <- lps_df_raw |> pull(gene)

  cytokine_df <- read.csv("ImmuneNoise/reference_gene_sets/InnateDB_cytokine_signalling.csv")
  cytokine_gene_list <- cytokine_df |>
    pull(name) |>
    unique()

  tlr_df <- read.csv("ImmuneNoise/reference_gene_sets/InnateDB_genes_TLR.csv") 
  tlr_gene_list <- tlr_df |>
    pull(name) |>
    unique()


  xue_response_raw <- readRDS("ImmuneNoise/reference_gene_sets/xue_response_genes.rds")
  ...
  immune_gene_list <- c(lps_list, cytokine_gene_list, tlr_gene_list, xue_gene_list[["IFNg"]], xue_gene_list[["IL10"]], xue_gene_list[["IL4"]], xue_gene_list[["TNF"]])
  immune_gene_list <- unique(immune_gene_list)

  df <- df |>
     mutate(Immune_response_gene = gene %in% immune_gene_list) 
  

  # tag genes not in df
  all_clusters <- df |> distinct(cluster_id, cell_type, tissue, age)
  all_combinations <- crossing(all_clusters, gene = immune_gene_list)
  missing_genes <- anti_join(all_combinations, df, 
                           by = c("cluster_id", "cell_type", "gene", "tissue", "age"))
  missing_genes <- missing_genes |>
  mutate(Immune_response_gene = TRUE,
...
tag_lps_genes <- function(df) {
  ...
  lps_df <- read_excel("ImmuneNoise/reference_gene_sets/Bhatt_2012_data.xlsx")
...
tag_sperm_genes <- function(df) {
  sperm_table <- read_excel('ImmuneNoise/reference_gene_sets/GO_gene_lists/GO_sperm_dna_condensation.xlsx')
...
tag_meiosis_genes <- function(df) {
  meiosis_table <- read_excel('ImmuneNoise/reference_gene_sets/GO_gene_lists/GO_G1_M_transition_meiosis.xlsx')
...
tag_oocyte_genes <- function(df) {
  ...
  oocyte_table <- read_excel('ImmuneNoise/reference_gene_sets/GO_gene_lists/GO_oocyte_maturation.xlsx')
...
tag_control_genes <- function(df) {
  # add all together (raise numbers)
  ...
  control_list <- c(oocyte_list, meiosis_list, sperm_list)

  df <- df |>
    mutate(Control_gene = gene %in% control_list)
   

  # tag genes not in df
  all_clusters <- df |> distinct(cluster_id, cell_type, tissue, age)
  all_combinations <- crossing(all_clusters, gene = control_list)
  missing_genes <- anti_join(all_combinations, df, 
                           by = c("cluster_id", "cell_type", "gene", "tissue", "age"))
  missing_genes <- missing_genes |>
    mutate(Control_gene = TRUE,
            Not_expressed  = TRUE,
            gmean = 0)
  df <- bind_rows(df, missing_genes)
...
tag_not_expresssed_genes <- function(df, data_source) {
  ...
  data_source = "facs"
   if (data_source == "facs") {
        print("Data source: FACS.")
        all_genes_list <- unique(readRDS("ImmuneNoise/reference_gene_sets/all_genes_facs.rds"))
    } else if (data_source == "droplet") {
      print("Data source: Droplet.")
      all_genes_list <- unique(readRDS("ImmuneNoise/reference_gene_sets/all_genes_droplet.rds"))
    } else {
      stop("Issue in data source or cell type renaming.")
    } 
...
tag_no_gene_set_genes <- function(df, existing_gene_sets, data_source) {
  ...
  saveRDS(df, paste0("ImmuneNoise/", data_source, '/data/gene_set_df.rds'))
