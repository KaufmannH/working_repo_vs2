...
# PART 1: comparing FACS and droplet with the entropy run already
# PART 2: comparing hepatocytes of TMS and PanSci
library(ggplot2)
library(patchwork)
...
heatmap_clustered <- function(combined_df) {
 all_cats <- c(
    ...
    bin_df_small <- bin_df_big |>
      ...
      summarise(gene_count_clusters = n(), .groups = "drop") 
    head(bin_df_small)
    ...
    heatmap_matrix <- heatmap_matrix[, all_cats] # arrange order of cols
  ...
  cluster_annotation <- bin_df_big |>
    ...
    arrange(heatmap_cluster, technology)
  row_order <- cluster_annotation$gene_tech
  heatmap_matrix <- heatmap_matrix[row_order, ] # arrange order of rows

  row_annot <- rowAnnotation(
    Technology = cluster_annotation$technology,
    Cluster = cluster_annotation$heatmap_cluster,
    col = list(
            Technology = c('facs' = 'darkgrey', 'droplet' = 'lightgrey'),
            Cluster = c('1' = '#111D4E', '2' = '#4472C4', '3' = '#9FBFEC')),
    annotation_name_side = 'top',
    simple_anno_size_adjust = TRUE)

  col_annot <- HeatmapAnnotation(
      Category = anno_simple(all_cats, col = col_vec),
...
heatmap_clustered_backup2 <- function(combined_df) {
