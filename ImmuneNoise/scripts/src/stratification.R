...
stratify_df <- function(df = NULL, data_source = NULL, cell_type_selection = NULL) {
  ...
  if (is.null(df)) { 
    print("No df given, loading saved df.")
      if (data_source == "facs") {
        ...
        df <- readRDS("ImmuneNoise/facs/data/gene_set_df.rds")
  ...
  if (data_source == "facs") {
      ...
      saveRDS(df, "ImmuneNoise/facs/data/strat_df.rds")
    } else if (data_source == "droplet") {
      ...
      saveRDS(df, "ImmuneNoise/droplet/data/strat_df.rds")
    }
    else if (data_source == "pansci") {
      print("Saved to: PanSci")
      #rename_cols <- endsWith(colnames(df_renamed), " gene")
      #colnames(df_renamed)[rename_cols] <- gsub(" ", "_", colnames(df)[rename_cols])
...
plot_expression_bins <- function(df, data_source) {
  ...
  strat_box <- ggplot(summary_table, aes(x = category, y = gmean, fill = category)) +
    ...
    ggsave(paste0('ImmuneNoise/', data_source, '/plots/3_m/Test_8/strat_expression_bins.png'), plot = strat_box,  width = 10, height = 14 )
...
plot_expression_bins_lps <- function(df, data_source) {
  ...
  ggsave(paste0('ImmuneNoise/', data_source, '/plots/3_m/Test_8/strat_expression_bins_lps.png'), plot = strat_box_lps,  width = 10, height = 14 )
...
plot_expression_bin_numbers <- function(df, data_source) {
  ...
  pdf(paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_8/strat_expression_numbers.pdf"),  width = 12, height = 15 )
  print(plot)
  dev.off()
}
...
plot_category_numbers <- function(df, data_source) {
  ...
  pdf( paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_8/strat_bar_all.pdf"),  width = 12, height = 15 )
  print(plot)
  dev.off()
}
...
plot_gene_set_numbers <- function(df, data_source) {
  ...
  ggsave(paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_8/strat_bar_gene_sets.png"),  width = 12, height = 15 )
...
plot_all_clusters <- function(df, data_source) {
  ...
  ggsave(paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_8/strat_per_gs.png"),  width = 16, height = 17 )
}
...
plot_gene_set_proportions <- function(df, data_source) {
  ...
  ggsave(paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_11/gene_set_proportions.png"), width = 45, height = 20 )
...
gene_set_proportions_compact <- function(df, data_source) {
  ...
  ggsave(paste0('ImmuneNoise/', data_source, "/plots/3_m/Test_11/gene_set_proportions_compact.png"),width = 32, height = 20 )   
}
